...
#define fin "\033[0m\n"
typedef struct elt
{ // declaration des structures de donnees : listes pour les pages, et l'livre pour l'index
  int numpage;
  struct elt *suivant;
} elt;
typedef elt *liste;
...
// declaration de l'info utilisé pour la partie 2
...
void RemplirInfo(livre root, information *values, int *index);
...
/*Fonction pour construire l'arbre binaire ordonné équilibré à partir d'un tableau
  La fonction Cree_arbre_equilibre utilise une approche de construction de l'arbre
  en utilisant un algorithme de division */
livre Cree_arbre_equilibre(information *values, int debut, int end);
...
// primitives de la file adapté aux nouveaux structures de données
...
// procédure pour afficher les chemins de l'arbre contenu dans la liste.
...
void main()
{
  ...
  { // quelque code pour l'esthetique du programme
    printf("%c", str[i]);
    fflush(stdout);
  ...
  while (partie != 0)
  {
    printf("\e[1;1H\e[2J"); // alternative de system("cls") pour éviter les problèmes de compatibilités (system("clear") & system("cls"))
    menuprincipale();
    scanf(" %c", &lettre);
    if (lettre == '1')
      partie = 1;
      ...
      parti1:
        ...
        case '1':
          ...
          if (e == 1)
          {
            e = 0;
            printf("\e[1;1H\e[2J");
            printf(Vert "Construction de l'arbre fait avec success....\nVous pouvez maintenant acceder a la deuxieme partie.\t" fin);
            goto ExitPartie1;
          }
          break;
        case '2':
        ...
        ExitPartie1:
          ...
          case '1':
            ...
            if (f == 1)
            {
              f = 0;
              printf("\e[1;1H\e[2J");
              printf(Vert "Arbre construit avec success....\nVous pouvez maintenant acceder a la troisieme partie.\t" fin Cyan "Merci pour votre comprehension" fin);
              goto PARTI3;
            }
            break;
          case '2':
          raffraichir:
            ...
            else
            {
              ...
              if (let == '1')
                ...
                affiche_struct_arbre(b);
              printf(Jaune "\ntapez 1 pour raffraichir" fin);
          ...
          Reessayer:
            ...
            else
            {
              ...
              if (recherche_mot_inserer_supprimer_page(&b, word, 0, 0, 0) == false)
              {
                printf(Rouge "Mot n'existe pas...\ntapez 5 pour Reessayer." fin);
                scanf("%d", &choix);
              }
              else
                supprimerMot(&b, word);
              printf(Vert "\nLa suppression faite avec success" fin);
              if (choix == 5)
                goto Reessayer;
            }
            sortir();
    ...
    else if (partie == 3)
    {
      ...
      {
        printf("\e[1;1H\e[2J");
        printf(Rouge "Arbre non construit..." fin Cyan " veuillez le construire d'abord\n Voici la partie 2 taper 1 pour construir l arbre " fin);
        f = 1;
        goto ExitPartie1;
      }
      else
...
void Const_Tree(livre *a, int x)
{
  ...
  *a = NULL;
  ...
  { // Choix d'un exemple
    do
    {
      printf("Generation d'un exemple avec 10 noeuds : \n\tEntrez le nombre de pages du livre (different de 0): ");
      scanf("%d", &taille);
      printf("\e[1;1H\e[2J");
    } while (taille == 0);

    for (int j = 0; j < 10; j++)
    { // initialisation du tableau dynamique de pages
...
bool recherche_mot_inserer_supprimer_page(livre *a, char *val, int nv, int x, int pg)
{
  ...
  if (*a != NULL)
  {
    ...
    { // on compare si les mots sont les mêmes
      if (x == 0)
      { // option de recherche
      ...
      { // option pour inserer un numero de page
        p = (*a)->page;
        ...
        if (p != NULL)
        {
          ...
          {
            t = malloc(sizeof(elt)); // allocation pour l'insertion de la page
            t->numpage = pg;
            t->suivant = NULL;
            p->suivant = t;
          }
      ...
      { // option pour supprimer un numero de page
        p = (*a)->page;
        if (p != NULL)
        {
          if (p->numpage == pg)
...
void RemplirInfo(livre root, information *values, int *index)
{
  ...
  RemplirInfo(root->succg, values, index);
  strcpy((values + (*index))->mot, root->mot);
  values[*index].page = root->page;
  (*index)++;
  RemplirInfo(root->succd, values, index);
}
...
livre Cree_arbre_equilibre(information *values, int debut, int end)
{
  ...
  root->succg = Cree_arbre_equilibre(values, debut, mid - 1);
  root->succd = Cree_arbre_equilibre(values, mid + 1, end);
...
livre Creer_Arbre_Ordonee(livre a)
{
  ...
  int indice = 0;
  RemplirInfo(a, val, &indice);
  tri_tableau(val, indice);
  return Cree_arbre_equilibre(val, 0, indice - 1);
}
// Fonction de parcours infixe de l'arbre binaire
void parcoursInfixe(index *root)
{
...
void branches(livre a, listechemin *l)
{
  *l = NULL;
  ...
  if (a != NULL)
  {
    ...
    while (!pilevide(P))
    {
      while (a->succd != NULL || a->succg != NULL)
      { // dépiler jusqu'à arriver à un noeud feuille
        if (a->succg != NULL)
        { // toujours y aller à la gauche si c'est possible
          empiler(&P, a->succg);
          a = a->succg;
          enfilermot(&F, a->mot);
        }
      ...
      { // cas ou la liste est vide
        *l = malloc(sizeof(eltlistechemin));
        temp.tete = NULL;
        temp.queue = NULL;    // création d'une nouvelle file
      ...
      depiler(&P, &b);
      defilermot(&F); // on dépile P sur b
      while (!pilevide(P))
      {
        depiler(&P, &a);
        if (a->succd == NULL || a->succd == b)
        { // on vérifie si on a déjà parcouru le chemin à droite à l'aide de b
          defilermot(&F);
          b = a;
        }
        else if (a->succd != b)
        { // au cas ou on trouve un fils droit, on sort de la boucle.
          empiler(&P, a);
          empiler(&P, a->succd);
...
{
  printf(" _______________________________________________________________________________________________\n");
  printf("|                                 Veuillez choisir la partie :                                  |\n");
  printf("|                                                                                               |\n");
